...
const getIconForProject = (title) => {
  ...
  if (t.includes('erp')) return <FaNetworkWired className="text-4xl text-cyan-400" />;
...
const ProjectTerminal = ({ project, index, onOpenModal }) => {
  ...
  return (
    <motion.div 
      ...
      className="glass-panel rounded-3xl overflow-hidden flex flex-col lg:flex-row group perspective-[1000px] border border-white/10 shadow-glass hover:shadow-glow hover:border-cyan-200 transition-all cursor-pointer relative"
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="absolute top-4 right-4 bg-cyan-900/30 text-cyan-400 px-3 py-1 rounded-full text-xs font-bold font-mono opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-4 group-hover:translate-x-0 duration-300 z-20 border border-cyan-500/30 shadow-glow">
      ...
      <div className="w-full lg:w-5/12 bg-slate-900/40 relative overflow-hidden flex items-center justify-center min-h-[300px] border-r border-white/10">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <motion.div 
            ...
            className="w-24 h-24 rounded-2xl border border-cyan-200 flex items-center justify-center bg-slate-800/80/40 shadow-glow mb-4 group-hover:shadow-cyan-200 transition-shadow duration-500 relative"
          >
          ...
          <span className="mt-4 font-mono text-xs text-cyan-400 uppercase tracking-widest font-bold">Interactive Module</span>
        </div>
      </div>
      ...
      <div className="w-full lg:w-7/12 p-8 flex flex-col bg-slate-900/20 relative">
        <div className="flex justify-between items-start mb-4">
          <div>
            <p className="font-mono text-purple-400 text-sm mb-1 font-semibold">{project.category}</p>
            <h3 className="text-2xl font-bold text-white group-hover:text-cyan-400 transition-colors">{project.title}</h3>
          </div>
        </div>

        <div className="bg-slate-900/40 rounded-2xl p-5 border border-white/5 flex-grow mb-6 shadow-glow group-hover:border-cyan-500/30 transition-colors">
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {project.features.map((feature, i) => (
              <li key={i} className="flex items-start text-xs text-gray-400 font-medium">
                <span className="text-cyan-400 mr-2 mt-[2px] font-bold">&gt;</span> {feature}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mt-auto">
          <div className="flex flex-wrap gap-2">
            {project.technologies.map((tech, i) => (
              <span key={i} className="text-xs font-mono text-cyan-300 bg-cyan-900/30 border border-cyan-500/30 px-3 py-1 rounded-full font-semibold">
                {tech}
              </span>
            ))}
          </div>
          ...
          <div className="flex gap-2 shrink-0">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors border ${project.liveUrl ? 'bg-cyan-900/300 text-white hover:bg-cyan-600 border-cyan-500' : 'bg-slate-800/80/50 text-gray-500 border-white/10 cursor-not-allowed hover:bg-slate-700/80/50'}`}
      ...
      >
        ...
        <div className="p-8 md:p-10 border-b border-white/5 bg-slate-900/40/50">
          <div className="flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-2xl bg-slate-800/80/40 border border-cyan-200 shadow-glow flex items-center justify-center mb-4">
               <FaRegFolderOpen className="text-3xl text-cyan-400" />
            </div>
            <p className="font-mono text-cyan-400 text-xs font-bold uppercase tracking-widest mb-2">Project Unavailable</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {project.technologies.map((tech, i) => (
                  <span key={i} className="text-xs font-mono text-cyan-300 bg-cyan-900/30 border border-cyan-500/30 px-2 py-1 rounded font-semibold">
                    {tech}
                  </span>
                ))}
              </div>
    ...
    <section id="projects" className="py-24 relative">
      <div className="container mx-auto px-6 md:px-12 max-w-6xl relative z-10">
        ...
        <div className="flex items-center mb-16">
          <span className="text-cyan-400 font-mono text-xl mr-4 tracking-widest">03.</span>
